...
parser.add_argument(
    "--noise_type", type=str, help="[pairflip, symmetric]", default="pairflip"
)
parser.add_argument(
    "--num_gradual",
    type=int,
...
for i in range(args.epoch_decay_start, args.n_epoch):
    alpha_plan[i] = (
        float(args.n_epoch - i)
        / (args.n_epoch - args.epoch_decay_start)
        * learning_rate
    )
    beta1_plan[i] = mom2
...
def main():
    ...
    with open(txtfile, "a") as myfile:
        myfile.write(
            ...
            + str(test_acc2)
            + " "
            + str(mean_pure_ratio1)
            + " "
            + str(mean_pure_ratio2)
            + "\n"
        )
