...
def loss_coteaching(y_1, y_2, t, forget_rate, ind, noise_or_not, label_smoothing=0.0):
    ...
    loss_1_update = F.cross_entropy(y_1[sel1], t[sel1], label_smoothing=label_smoothing)
    loss_2_update = F.cross_entropy(y_2[sel2], t[sel2], label_smoothing=label_smoothing)
...
def main():
    ...
    print(
        f"Epoch 0 - Test Acc: M1 {test_acc1:.2f}%, M2 {test_acc2:.2f}% | Test Loss: L1 {test_loss1:.4f}, L2 {test_loss2:.4f}"
    )
    with open(txtfile, "a") as f:
        f.write(
            f"0 0.0000 0.0000 {test_acc1:.2f} {test_acc2:.2f} {test_loss1:.4f} {test_loss2:.4f} 0.00 0.00\n"
        )
    ...
    for epoch in range(1, args.n_epoch):
        ...
        print(
            f"Epoch {epoch} - Train M1:{tr_acc1:.2f}%, M2:{tr_acc2:.2f}% | Test M1:{te_acc1:.2f}%, M2:{te_acc2:.2f}% | Test Loss: L1 {te_loss1:.4f}, L2 {te_loss2:.4f}"
        )
        with open(txtfile, "a") as f:
            f.write(
                f"{epoch} {tr_acc1:.4f} {tr_acc2:.4f} {te_acc1:.2f} {te_acc2:.2f} {te_loss1:.4f} {te_loss2:.4f} {pr1:.2f} {pr2:.2f}\n"
            )
